...
- **MemoQ XLIFF: мусорные внутренние теги в отчётах** — `<ph>`/`<bpt>`/`<ept>`/`<it>`/`<x>` в `<source>`/`<target>` больше не разворачиваются как сырой текст вида `<mq:rxt displaytext="&lt;p&gt;" val="&lt;p&gt;">`. Вместо этого подставляется компактный плейсхолдер `{id}` (например `{1}Hello{2}`). Поиск `context`/`note` ограничен прямыми детьми `<trans-unit>`, чтобы исторические версии из `<mq:minorversions>` не попадали в текущий сегмент.
- **Устаревшие тесты** — `test_main_window_creation` подтянут под новый минимальный размер окна (900×620 из v2.4); `test_html_reporter_generates_report` обновлён под `data-type`/`data-filter`-атрибуты гранулярных фильтров из v2.3.
